  ...
  function togglePacer() {
    if (_pacerRunning) {
      ...
      _pacerRunning = true;
      _pacerPhaseIdx = 0;
      _pacerPhaseStart = null;
      _pacerSyncBtn();
      _pacerRaf = requestAnimationFrame(_pacerTick);
    }
  }

  function _pacerStop() {
    _pacerRunning = false;
    if (_pacerRaf) { cancelAnimationFrame(_pacerRaf); _pacerRaf = null; }
    _pacerPhaseStart = null;
    const preset = _PACER_PRESETS[_pacerPreset];
    if (preset) {
      const startVert = preset.phases[0].from;
      _pacerSetNode(_PACER_VERTS[startVert]);
    }
    _pacerHighlight(-1);
    _pacerSyncBtn();
  }

  function setPacerPreset(val) {
    _pacerSetPreset(val, true);
  }

  function _pacerSetPreset(val, restart) {
    if (!_PACER_PRESETS[val]) return;
    _pacerPreset = val;
    _pacerSyncPresetBtns();
    if (_pacerRunning || restart) {
      _pacerStop();
    }
    const preset = _PACER_PRESETS[val];
    _pacerSetNode(_PACER_VERTS[preset.phases[0].from]);
    _pacerHighlight(-1);
  }

  function _pacerSetNode(v) {
    const n = document.getElementById('pacerNode');
    if (n) { n.setAttribute('cx', v.x); n.setAttribute('cy', v.y); }
  }

  function _pacerHighlight(vertIdx) {
    for (let i = 0; i < 3; i++) {
      const el = document.getElementById('pacerLbl' + i);
      if (el) el.setAttribute('opacity', i === vertIdx ? '1' : '0.38');
    }
    const phaseNames = ['Inhale', 'Hold', 'Exhale'];
    const phaseEl = document.getElementById('pacerPhaseDisplay');
    if (phaseEl) phaseEl.textContent = vertIdx >= 0 ? phaseNames[vertIdx] : '';
  }

  function _pacerSyncBtn() {
    const btn = document.getElementById('pacerStartBtn');
    if (btn) btn.textContent = _pacerRunning ? 'Pause' : 'Start';
  }

  function _pacerSyncPresetBtns() {
    document.querySelectorAll('.pacer-preset-btn').forEach(b => {
      b.classList.toggle('active', b.dataset.preset === _pacerPreset);
    });
  }
  ...
  function _pacerTick(now) {
    if (!_pacerRunning) return;
    const preset = _PACER_PRESETS[_pacerPreset];
    if (!preset) { _pacerStop(); return; }
    if (_pacerPhaseStart === null) _pacerPhaseStart = now;

    const phase = preset.phases[_pacerPhaseIdx];
    const elapsed = now - _pacerPhaseStart;
    const totalPhase = phase.move + phase.dwell;

    if (elapsed < phase.move) {
      const t = _easeInOut(elapsed / phase.move);
      const a = _PACER_VERTS[phase.from];
      const b = _PACER_VERTS[phase.to];
      _pacerSetNode({ x: a.x + (b.x - a.x) * t, y: a.y + (b.y - a.y) * t });
      _pacerHighlight(elapsed < phase.move * 0.5 ? phase.from : phase.to);
    } else if (elapsed < totalPhase) {
      _pacerSetNode(_PACER_VERTS[phase.to]);
      _pacerHighlight(phase.to);
    } else {
      _pacerPhaseIdx = (_pacerPhaseIdx + 1) % preset.phases.length;
      _pacerPhaseStart = now;
    }
  ...
  const libUiState = {
    tab: 'all',                // 'all' | 'books' | 'people' | 'podcasts' | 'breathwork' | 'meditations'
    search: '',
